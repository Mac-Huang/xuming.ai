...
try{
 ...
 await page.reload();
 await page.waitForFunction(()=>document.querySelector('#connect').textContent==='Disconnect GitHub' && document.querySelector('#editor').contentEditable==='true');
 ...
 await page.reload();await page.waitForFunction(()=>document.querySelector('#editor').contentEditable==='true');
 assert.equal(await page.locator('#connect').innerText(),'Connect GitHub');
 ...
 console.log('PASS: shared style, calendar navigation, Unicode draft recovery, token lifecycle, remembered reconnection, post-refresh autosave, disconnect, expiration, autosave, image ordering, conflict/reload/resolve, rejected-save retry, HTML sanitization, mobile layout.');
