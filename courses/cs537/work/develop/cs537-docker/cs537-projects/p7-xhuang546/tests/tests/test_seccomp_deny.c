...
int main(void) {
    ...
    char cwd[PATH_MAX];
    int rc;
    const char *suffix = "/tmp/test_11/rootfs";
    ...
    if (ct_init(&cfg, rootfs) != 0) {
        perror("ct_init deny case");
        return 1;
    }
    if (ct_disallow_all(&cfg) != 0 || allow_common_runtime_syscalls(&cfg) != 0) {
        perror("seccomp deny setup");
        return 1;
    }

    rc = ct_run(&cfg, argv);
    if (rc != 13) {
        fprintf(stderr, "deny case returned %d, expected 13\n", rc);
        return 1;
    }

    printf("PASS seccomp_denies_unallowed_syscall\n");
    return 0;
}
