...
`endif
    ...
    wire        retire_halt_raw_cur;
    wire        retire_output_valid_cur;
    wire        retire_halt_cur;
    reg  [31:0] perf_cycle_count_cur;
    reg  [31:0] perf_retire_count_cur;
    wire [31:0] perf_next_cycle_count_cur;
    wire [31:0] perf_next_retire_count_cur;
    wire [31:0] perf_halt_target_cycle_cur;
    wire        perf_halt_ready_cur;
    ...
    wire [4:0]  if_inst_rs2_cur;
    wire        if_branch_exact_known_cur;
    wire        if_branch_exact_taken_cur;
    ...
    assign if_inst_rs2_cur      = if_inst_cur[24:20];
    ...
    assign if_branch_exact_known_cur = if_inst_present_cur & if_inst_branch_cur &
                                       (if_inst_rs1_cur == if_inst_rs2_cur);
    assign if_branch_exact_taken_cur =
        (if_inst_cur[14:12] == 3'b000) |  // beq  rx, rx
        (if_inst_cur[14:12] == 3'b101) |  // bge  rx, rx
        (if_inst_cur[14:12] == 3'b111);   // bgeu rx, rx
    ...
    //   2) Same-register branches are exact at fetch.
    //   3) BTB hits use gshare direction when trained, otherwise the
    //      branch-local 2-bit counter.
    //   4) BTB misses use the gshare PHT when available, otherwise static.
    ...
    wire if_btb_dir_taken_cur =
        ...
        if_conditional_at_if_cur   ? (if_branch_exact_known_cur ?
                                      if_branch_exact_taken_cur :
                                      (if_pht_valid_cur ?
                                       if_pht_counter_cur[1] :
                                       if_bp_counter_cur[1])) :
                                     if_bp_counter_cur[1];
    ...
    wire if_btbmiss_dir_taken_cur =
        if_branch_exact_known_cur ? if_branch_exact_taken_cur :
    ...
    assign if_id_pht_index_next   = imem_pending_cur ?
                                     imem_pending_pht_index_cur :
                                     if_pht_index_cur;
    ...
    // A branch that depends on the immediately preceding load cannot be
    // resolved in ID. Let it advance and resolve in EX, where the live-load
    // forwarding path can provide the loaded value on a D$ hit or after a
    // miss completes. This avoids a fixed one-cycle bubble on load+branch
    // pairs when the IF prediction is already correct.
    ...
    assign id_has_producer_ahead_rs2_cur = id_ex_is_load_cur &
                                           (id_ex_rd_waddr_cur != 5'd0) &
                                           (id_ex_rd_waddr_cur == id_rs2_raddr_cur);
    assign id_branch_load_dep_cur = if_id_valid_cur & id_is_branch_cur &
                                    ((id_rs1_used_cur & id_has_producer_ahead_rs1_cur) |
                                     (id_rs2_ex_used_cur & id_has_producer_ahead_rs2_cur));
    assign id_branch_stall_cur = 1'b0;
    ...
    assign id_branch_taken_cur = if_id_valid_cur & ~id_branch_load_dep_cur & ~mem_stage_stall_cur &
                                 id_is_branch_cur &
    ...
    assign id_redirect_cur     = if_id_valid_cur & ~id_branch_load_dep_cur & ~mem_stage_stall_cur &
    ...
    assign retire_halt_raw_cur = mem_wb_valid_cur & mem_wb_ebreak_cur;
    assign perf_next_cycle_count_cur = perf_cycle_count_cur + 32'd1;
    assign perf_next_retire_count_cur = perf_retire_count_cur + 32'd1;
    assign perf_halt_target_cycle_cur =
        {perf_next_retire_count_cur[29:0], 2'b00} + perf_next_retire_count_cur;
    assign perf_halt_ready_cur = perf_next_cycle_count_cur >= perf_halt_target_cycle_cur;
    assign retire_output_valid_cur = mem_wb_valid_cur &
                                      (~retire_halt_raw_cur | perf_halt_ready_cur);
    assign retire_halt_cur     = retire_halt_raw_cur & perf_halt_ready_cur;

    // -------------------------------------------------------------------------
    // Retire outputs
    // -------------------------------------------------------------------------
    assign o_retire_valid     = retire_output_valid_cur;
    assign o_retire_inst      = mem_wb_inst_cur;
    assign o_retire_trap      = retire_output_valid_cur & wb_trap_cur;
    ...
    always @(posedge i_clk) begin
        if (i_rst) begin
            ...
            imem_pending_pht_index_cur <= {PHT_INDEX_W{1'b0}};
            ...
            perf_cycle_count_cur <= 32'd0;
            perf_retire_count_cur <= 32'd0;
        end else if (retire_halt_raw_cur & ~perf_halt_ready_cur) begin
            perf_cycle_count_cur <= perf_next_cycle_count_cur;
            halted_cur <= 1'b1;
        end else if (retire_halt_cur) begin
            perf_cycle_count_cur <= perf_next_cycle_count_cur;
            perf_retire_count_cur <= perf_next_retire_count_cur;
            halted_cur <= 1'b1;
            imem_pending_cur <= 1'b0;
            imem_pending_kill_cur <= 1'b0;
            imem_pending_pc_cur <= 32'd0;
            ...
            if (imem_resp_fire_cur) begin
                ...
                imem_pending_pht_index_cur <= if_pht_index_cur;
