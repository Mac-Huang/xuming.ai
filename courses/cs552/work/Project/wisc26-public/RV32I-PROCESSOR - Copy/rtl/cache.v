...
module cache #(
    ...
    //   LINE_WIDE=0: 16B lines, 64 sets (default, for D-cache locality).
    //   LINE_WIDE=1: 32B lines, 32 sets.
    //   LINE_WIDE=2: 64B lines, 16 sets (best for sequential I-cache fetch).
    // All store 4 KB.
    localparam O     = (LINE_WIDE == 2) ? 6 :
                       (LINE_WIDE == 1) ? 5 : 4;
    localparam S     = (LINE_WIDE == 2) ? 4 :
                       (LINE_WIDE == 1) ? 5 : 6;
    localparam DEPTH = (LINE_WIDE == 2) ? 16 :
                       (LINE_WIDE == 1) ? 32 : 64;
    localparam T     = 32 - O - S;
    localparam D     = (LINE_WIDE == 2) ? 16 :
                       (LINE_WIDE == 1) ? 8 : 4;
    ...
    reg                    store_fill_valid_cur;
    reg [T - 1:0]          store_fill_tag_cur;
    reg [S - 1:0]          store_fill_set_cur;
    reg [3:0]              store_fill_mask_cur;
    reg [31:0]             store_fill_datas0_cur;
    reg [31:0]             store_fill_datas1_cur;
    reg [31:0]             store_fill_datas2_cur;
    reg [31:0]             store_fill_datas3_cur;
    ...
    wire        refill_req_matches_rsp_cur = (req_tag == rsp_tag_cur) &
                                             (req_set == rsp_set_cur);
    wire [31:0] store_fill_req_data_cur =
        (req_word[1:0] == 2'd0) ? store_fill_datas0_cur :
        (req_word[1:0] == 2'd1) ? store_fill_datas1_cur :
        (req_word[1:0] == 2'd2) ? store_fill_datas2_cur :
                                  store_fill_datas3_cur;
    wire store_fill_req_word_valid_cur =
        store_fill_valid_cur & (store_fill_tag_cur == req_tag) &
        (store_fill_set_cur == req_set) &
        ((req_word[1:0] == 2'd0) ? store_fill_mask_cur[0] :
         (req_word[1:0] == 2'd1) ? store_fill_mask_cur[1] :
         (req_word[1:0] == 2'd2) ? store_fill_mask_cur[2] :
                                   store_fill_mask_cur[3]);
    wire [31:0] store_fill_refill_data_cur =
        (refill_rsp_word_abs[1:0] == 2'd0) ? store_fill_datas0_cur :
        (refill_rsp_word_abs[1:0] == 2'd1) ? store_fill_datas1_cur :
        (refill_rsp_word_abs[1:0] == 2'd2) ? store_fill_datas2_cur :
                                             store_fill_datas3_cur;
    wire store_fill_refill_word_valid_cur =
        store_fill_valid_cur & (store_fill_tag_cur == rsp_tag_cur) &
        (store_fill_set_cur == rsp_set_cur) &
        ((refill_rsp_word_abs[1:0] == 2'd0) ? store_fill_mask_cur[0] :
         (refill_rsp_word_abs[1:0] == 2'd1) ? store_fill_mask_cur[1] :
         (refill_rsp_word_abs[1:0] == 2'd2) ? store_fill_mask_cur[2] :
                                              store_fill_mask_cur[3]);
    wire [31:0] refill_rsp_data_cur = store_fill_refill_word_valid_cur ?
                                      store_fill_refill_data_cur : i_mem_rdata;
    ...
    wire        read_miss  = i_req_ren & ~hit & ~store_fill_req_word_valid_cur &
                             ~early_restart_match_cur &
                             ~refill_partial_hit_cur;
    wire        write_miss = i_req_wen & ~hit;
    wire        wb_idle_send_possible_cur = (state_cur == STATE_IDLE) &
                                            (wb_count_cur != WB_COUNT_ZERO) &
                                            i_mem_ready;
    wire           store_fill_same_line_cur = store_fill_valid_cur &
                                              (store_fill_tag_cur == req_tag) &
                                              (store_fill_set_cur == req_set);
    // A full-word store miss does not need an old-word refill; write it
    // through the buffer and let later reads/refills forward it.
    wire        write_full_miss_accept_cur = (state_cur == STATE_IDLE) &
                                             ~pending_req_cur &
                                             (LINE_WIDE == 0) &
                                             i_req_wen &
                                             ~hit &
                                             (i_req_mask == 4'b1111) &
                                             ((wb_count_cur != WB_COUNT_FULL) |
                                              wb_idle_send_possible_cur) &
                                             (~store_fill_valid_cur |
                                              store_fill_same_line_cur);
    wire [3:0] store_fill_word_bit_cur =
        (req_word[1:0] == 2'd0) ? 4'b0001 :
        (req_word[1:0] == 2'd1) ? 4'b0010 :
        (req_word[1:0] == 2'd2) ? 4'b0100 : 4'b1000;
    wire [3:0] store_fill_mask_after_cur =
        (store_fill_same_line_cur ? store_fill_mask_cur : 4'b0000) |
        store_fill_word_bit_cur;
    wire           store_fill_complete_cur =
        write_full_miss_accept_cur & (&store_fill_mask_after_cur);
    wire           rsp_matches_store_fill_cur = store_fill_valid_cur &
                                                (store_fill_tag_cur == rsp_tag_cur) &
                                                (store_fill_set_cur == rsp_set_cur);
    wire [31:0] store_fill_line_data0_cur =
        (req_word[1:0] == 2'd0) ? i_req_wdata : store_fill_datas0_cur;
    wire [31:0] store_fill_line_data1_cur =
        (req_word[1:0] == 2'd1) ? i_req_wdata : store_fill_datas1_cur;
    wire [31:0] store_fill_line_data2_cur =
        (req_word[1:0] == 2'd2) ? i_req_wdata : store_fill_datas2_cur;
    wire [31:0] store_fill_line_data3_cur =
        (req_word[1:0] == 2'd3) ? i_req_wdata : store_fill_datas3_cur;
    wire        pending_needs_refill = pending_req_cur & ~pending_hit;
    wire        cpu_miss_req_cur = (state_cur == STATE_IDLE) &
                                   (pending_needs_refill | read_miss |
                                    (write_miss & ~write_full_miss_accept_cur));
    ...
    wire        wb_push_cur = write_hit_accept_cur |
                              write_full_miss_accept_cur |
                              write_req_commit_cur;
    wire [31:0] wb_push_addr_cur = (write_hit_accept_cur |
                                    write_full_miss_accept_cur) ?
                                   i_req_addr : miss_word_addr;
    wire [31:0] wb_push_data_cur = write_hit_accept_cur ? merged_write_word :
                                   write_full_miss_accept_cur ? i_req_wdata :
    ...
    always @(posedge i_clk) begin
        if (i_rst) begin
            ...
            store_fill_valid_cur <= 1'b0;
            store_fill_tag_cur <= {T{1'b0}};
            store_fill_set_cur <= {S{1'b0}};
            store_fill_mask_cur <= 4'b0000;
            valid0 <= {DEPTH{1'b0}};
            valid1 <= {DEPTH{1'b0}};
            valid2 <= {DEPTH{1'b0}};
            valid3 <= {DEPTH{1'b0}};
            ...
            case (state_cur)
                STATE_IDLE: begin
                    ...
                    end else if (write_full_miss_accept_cur) begin
                        // Write around the cache. The write buffer forwards
                        // later reads and refills merge queued writes. If a
                        // store burst covers the whole line, install that line
                        // from the store data without a memory refill.
                        if (store_fill_complete_cur) begin
                            case (victim_way)
                                2'd0: begin
                                    datas0[req_set][0] <= store_fill_line_data0_cur;
                                    datas0[req_set][1] <= store_fill_line_data1_cur;
                                    datas0[req_set][2] <= store_fill_line_data2_cur;
                                    datas0[req_set][3] <= store_fill_line_data3_cur;
                                    tags0[req_set] <= req_tag;
                                    valid0[req_set] <= 1'b1;
                                end
                                2'd1: begin
                                    datas1[req_set][0] <= store_fill_line_data0_cur;
                                    datas1[req_set][1] <= store_fill_line_data1_cur;
                                    datas1[req_set][2] <= store_fill_line_data2_cur;
                                    datas1[req_set][3] <= store_fill_line_data3_cur;
                                    tags1[req_set] <= req_tag;
                                    valid1[req_set] <= 1'b1;
                                end
                                2'd2: begin
                                    datas2[req_set][0] <= store_fill_line_data0_cur;
                                    datas2[req_set][1] <= store_fill_line_data1_cur;
                                    datas2[req_set][2] <= store_fill_line_data2_cur;
                                    datas2[req_set][3] <= store_fill_line_data3_cur;
                                    tags2[req_set] <= req_tag;
                                    valid2[req_set] <= 1'b1;
                                end
                                default: begin
                                    datas3[req_set][0] <= store_fill_line_data0_cur;
                                    datas3[req_set][1] <= store_fill_line_data1_cur;
                                    datas3[req_set][2] <= store_fill_line_data2_cur;
                                    datas3[req_set][3] <= store_fill_line_data3_cur;
                                    tags3[req_set] <= req_tag;
                                    valid3[req_set] <= 1'b1;
                                end
                            endcase
                            repl_way[req_set] <= victim_way + 2'd1;
                            store_fill_valid_cur <= 1'b0;
                            store_fill_mask_cur <= 4'b0000;
                        end else begin
                            store_fill_valid_cur <= 1'b1;
                            store_fill_tag_cur <= req_tag;
                            store_fill_set_cur <= req_set;
                            store_fill_mask_cur <= store_fill_mask_after_cur;
                            case (req_word[1:0])
                                2'd0: store_fill_datas0_cur <= i_req_wdata;
                                2'd1: store_fill_datas1_cur <= i_req_wdata;
                                2'd2: store_fill_datas2_cur <= i_req_wdata;
                                default: store_fill_datas3_cur <= i_req_wdata;
                            endcase
                        end
                    end else if (write_hit_accept_cur) begin
                        if (store_fill_same_line_cur)
                            store_fill_valid_cur <= 1'b0;
                        case (hit_way)
                ...
                STATE_REFILL_REQ: begin
                    ...
                    if (i_mem_valid) begin
                        case (rsp_way_cur)
                            2'd0: datas0[rsp_set_cur][refill_rsp_word_abs] <= refill_rsp_data_cur;
                            2'd1: datas1[rsp_set_cur][refill_rsp_word_abs] <= refill_rsp_data_cur;
                            2'd2: datas2[rsp_set_cur][refill_rsp_word_abs] <= refill_rsp_data_cur;
                            default: datas3[rsp_set_cur][refill_rsp_word_abs] <= refill_rsp_data_cur;
                        endcase
                        ...
                        if (refill_rsp_word_cur == 4'd0 && ~rsp_is_prefetch_cur &&
                            ...
                            early_restart_data_cur  <= refill_rsp_data_cur;
                        end

                        if (refill_rsp_word_cur == REFILL_LAST_WORD) begin
                            if (rsp_matches_store_fill_cur)
                                store_fill_valid_cur <= 1'b0;
                            case (rsp_way_cur)
                                2'd0: begin
    ...
    assign o_busy = (((state_cur == STATE_REFILL_REQ) & ~rsp_is_prefetch_cur) |
                    ...
                    ~early_restart_match_cur &
                    ~refill_partial_hit_cur;
    assign o_res_rdata = store_fill_req_word_valid_cur ? store_fill_req_data_cur :
                         early_restart_match_cur  ? refill_rsp_data_cur :
