...
The generated knowledge version is a hash of the content. The browser fetches it with cache revalidation, replacing the old chunk-count-only embedding cache. The September 23 build contains 170 records, including all 24 listed projects, actual publications, current coursework, and full prose from the listed blogs.
